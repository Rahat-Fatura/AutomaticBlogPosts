...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'cyberguard');
...
$merlin_setup = get_option( 'merlin_' . cyberguard()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'cyberguard');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'cyberguard');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'cyberguard');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'cyberguard' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'cyberguard' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'cyberguard' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new cyberguard_Admin_Plugins;
