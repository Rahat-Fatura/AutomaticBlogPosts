 ...
 * Google Fonts
*/
function cyberguard_fonts_url() {
    $fonts_url = '';
    $fonts     = array();
    $subsets   = 'latin,latin-ext';

    if ( 'off' !== _x( 'on', 'DM+Sans font: on or off', 'cyberguard' ) ) {
        $fonts[] = 'DM+Sans:ital,opsz,wght@0,9..40,100..1000;1,9..40,100..1000&display=swap';
    }

    if ( 'off' !== _x( 'on', 'Urbanist font: on or off', 'cyberguard' ) ) {
 ...
 * Get page ID by Slug
*/
function cyberguard_get_id_by_slug($slug, $post_type){
 ...
 **/
function cyberguard_content_by_slug($slug, $post_type){
    $content = cyberguard_get_content_by_slug($slug, $post_type);

    $id = cyberguard_get_id_by_slug($slug, $post_type);
 ...
 * Get content by slug
 **/
function cyberguard_get_content_by_slug($slug, $post_type){
    $content = get_posts(
        array(
            'name'      => $slug,
            'post_type' => $post_type
 ...
 * Custom Comment List
 */
function cyberguard_comment_list( $comment, $args, $depth ) {
    if ( 'div' === $args['style'] ) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
 ...
 * Paginate Links
 */
function cyberguard_ajax_paginate_links($link){
 ...
 * RGB Color
 */
function cyberguard_hex_rgb($color) {
 ...
 * Image Size Crop
 */
if(!function_exists('cyberguard_get_image_by_size')){
    function cyberguard_get_image_by_size( $params = array() ) {
 ...
 * Search Form
 */
function cyberguard_header_mobile_search_form() { 
    $search_mobile = cyberguard()->get_theme_opt( 'search_mobile', false );
    $search_placeholder_mobile = cyberguard()->get_theme_opt( 'search_placeholder_mobile' );
    if($search_mobile) : ?>
...
if(function_exists( 'pxl_register_shortcode' )) {
    function cyberguard_year_shortcode() {
        ob_start(); ?>
            <span><?php the_date('Y'); ?></span>
        <?php $output = ob_get_clean();
        return $output;
    }
    pxl_register_shortcode('pxl_year', 'cyberguard_year_shortcode');
}

/* Highlight Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function cyberguard_text_highlight_shortcode( $atts = array() ) {
    ...
    pxl_register_shortcode('highlight', 'cyberguard_text_highlight_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function cyberguard_pie_chart_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'percentage' => '',
         'title' => '',
     ), $atts));
        $primary_color = cyberguard_get_opt( 'primary_color', '#4b83fc' );
    ...
    pxl_register_shortcode('ct_pie_chart', 'cyberguard_pie_chart_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function cyberguard_text_highlight_shortcode_editor( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
        ), $atts));
    ...
    pxl_register_shortcode('pxl_highlight', 'cyberguard_text_highlight_shortcode_editor');
}

/* Typewriter Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function cyberguard_text_typewriter_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
        ), $atts));
    ...
    pxl_register_shortcode('typewriter', 'cyberguard_text_typewriter_shortcode');
}

/* Button Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function cyberguard_btn_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'text' => '',
         'style' => 'btn-slider1',
         'icon_class' => '',
    ...
    pxl_register_shortcode('pxl_button', 'cyberguard_btn_shortcode');
}

/* Gallery Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function cyberguard_gallery_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'link' => '#',
         'images_id' => '',
         'cols' => '2',
        ...
        foreach ($pxl_images as $key => $img_id) :
            ...
            <div class="pxl--item">
                <div class="pxl--item-inner <?php if($key == 1 && !empty($link) && $btn_video == 'yes' ) { echo 'video-active'; } ?>">
                    <a href="<?php echo esc_url($thumbnail_url); ?>"><?php echo cyberguard_html($thumbnail); ?></a>
    ...
    pxl_register_shortcode('pxl_gallery', 'cyberguard_gallery_shortcode');
}

/* Addd shortcode Video button */
if(function_exists( 'pxl_register_shortcode' )) {
    function cyberguard_video_button_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
         'link' => '',
         'text' => '',
         'class' => 'style1',
    ...
    pxl_register_shortcode('pxl_video_button', 'cyberguard_video_button_shortcode');
}

//**
 /* Custom Widget Categories
 */
add_filter('wp_list_categories', 'cyberguard_wg_category_count');
function cyberguard_wg_category_count($output) {
 ...
 */
add_filter('get_archives_link', 'cyberguard_archive_count_span');
function cyberguard_archive_count_span($links) {
    $dir = is_rtl() ? 'pxl-left' : 'pxl-right';
 ...
 */
add_filter('wp_list_categories', 'cyberguard_wc_cat_count_span');
function cyberguard_wc_cat_count_span($links) {
    $dir = is_rtl() ? 'pxl-left' : 'pxl-right';
 ...
 */
function cyberguard_get_mega_menu_builder_id(){
    $mn_id = [];
 ...
 */
function cyberguard_get_page_popup_builder_id(){
    $pp_id = [];
...
function cyberguard_mouse_move_animation() { 
    $mouse_move_animation = cyberguard()->get_theme_opt('mouse_move_animation', false); 
    $mouse_move_style = cyberguard()->get_theme_opt( 'mouse_move_style', 'style-default' );
    if($mouse_move_animation) {
        wp_enqueue_script( 'cyberguard-cursor', get_template_directory_uri() . '/assets/js/libs/cursor.js', array( 'jquery' ), '1.0.0', true ); ?>  
 ...
 */
add_action( 'show_user_profile', 'cyberguard_user_fields' );
add_action( 'edit_user_profile', 'cyberguard_user_fields' );
function cyberguard_user_fields($user){
    ...
    ?>
    <h3><?php esc_html_e('Theme Custom', 'cyberguard'); ?></h3>
    <table class="form-table">
        <tr>
            <th><label for="user_position"><?php esc_html_e('Position', 'cyberguard'); ?></label></th>
            <td>
...
add_action( 'personal_options_update', 'cyberguard_save_user_custom_fields' );
add_action( 'edit_user_profile_update', 'cyberguard_save_user_custom_fields' );
function cyberguard_save_user_custom_fields( $user_id )
{
...
function cyberguard_get_user_theme() {
...
/* Author Social */
function cyberguard_get_user_social() {
 ...
 * Start - Cookie Policy
 */
function cyberguard_cookie_policy() {
    $cookie_policy = cyberguard()->get_theme_opt('cookie_policy', 'hide');
    $cookie_policy_description = cyberguard()->get_theme_opt('cookie_policy_description');
    $cookie_policy_btntext = cyberguard()->get_theme_opt('cookie_policy_btntext');
    $cookie_policy_link = get_permalink(cyberguard()->get_theme_opt('cookie_policy_link')); 
 ...
 */
function cyberguard_subscribe_popup() {
    $subscribe = cyberguard()->get_theme_opt('subscribe', 'hide');
    $subscribe_layout = cyberguard()->get_theme_opt('subscribe_layout');
    $popup_effect = cyberguard()->get_theme_opt('popup_effect', 'fade');
    $args = [
 ...
 * Start - Page Popup
 */
function cyberguard_page_popup() {
    $post_list = array();
    $args = array(
