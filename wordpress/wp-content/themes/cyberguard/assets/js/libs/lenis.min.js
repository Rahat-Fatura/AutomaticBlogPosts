...
;(function($) {
  ...
  $(document).ready(function() {
    ...
    // Manage hover events
    const selectors = ['.pxl-hidden-panel-popup', '.pxl-page-popup', '.pxl-nav-menu .pxl-menu-primary .pxl-megamenu > .sub-menu'];

    $.each(selectors, function(i, selector) {
      $(selector).on("mouseenter", function() {
        if (lenis) lenis.pause(); // Use pause instead of destroy
      });

      $(selector).on("mouseleave", function() {
        if (lenis) lenis.resume(); // Resume the instance instead of creating a new one
      });
    });
  });
})(jQuery);