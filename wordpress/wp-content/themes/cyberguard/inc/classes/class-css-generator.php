...
class cyberguard_CSS_Generator {
	/**
     * @access protected
     * @var scssc
     */
	...
	function __construct() {
		$this->opt_name = cyberguard()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'cyberguard_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'cyberguard_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->cyberguard_generate_file_options();
		} );
	}

	function cyberguard_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->cyberguard_generate_file_options();
			$this->cyberguard_generate_file();
		}
	}

    function cyberguard_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->cyberguard_options_output() )
        ) );
    }

	function cyberguard_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function cyberguard_options_output() {
		$theme_colors                    = cyberguard_configs('theme_colors');
        $links                           = cyberguard_configs('link');
        $gradients                       = cyberguard_configs('gradient');
		ob_start();

		printf('$cyberguard_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$cyberguard_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
    ...
    /* Inline CSS */
    function cyberguard_enqueue() {
        $css = $this->cyberguard_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function cyberguard_inline_css() {
        ob_start();
