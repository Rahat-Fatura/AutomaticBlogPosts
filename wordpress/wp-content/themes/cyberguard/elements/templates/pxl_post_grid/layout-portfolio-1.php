...
if( count($posts) <= 0){
    echo '<div class="pxl-no-post-grid">'.esc_html__( 'No Post Found', 'cyberguard' ). '</div>';
    return;
} ?>
...
<div <?php pxl_print_html($widget->get_render_attribute_string( 'wrapper' )) ?> data-layout="<?php echo esc_attr($settings['data_layout']); ?>">
    ...
    <div class="<?php echo esc_attr($grid_class); ?>" data-gutter="15">
        ...
        <?php cyberguard_get_post_grid($posts, $load_more); ?>
    </div>
    <?php if ($pagination_type == 'pagination') { ?>
        <div class="pxl-grid-pagination">
            <?php cyberguard()->page->get_pagination($query, true); ?>
        </div>
    <?php } ?>
    <?php if (!empty($next_link) && $pagination_type == 'loadmore') { ?>
        <div class="pxl-load-more">
            <span class="btn-grid-loadmore btn btn-default">
                <span class="pxl-loadmore-text pxl--btn-text"><?php echo esc_html__('Load More', 'cyberguard') ?></span>
